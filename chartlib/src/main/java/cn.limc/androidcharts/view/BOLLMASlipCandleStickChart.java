...
public class BOLLMASlipCandleStickChart extends MASlipCandleStickChart {
	...
	protected void calcDataValueRange() {
		...
		// 逐条输出MA线
		int dataSize = this.bandData.size();
	...
	protected void drawAreas(Canvas canvas) {
		...
		LineEntity<DateValueEntity> line1 = bandData
				.get(0);
		LineEntity<DateValueEntity> line2 = bandData
				.get(1);
	...
	protected void drawBandBorder(Canvas canvas) {
		...
		for (int i = 0; i < bandData.size(); i++) {
			LineEntity<DateValueEntity> line = bandData
					.get(i);
			if (line == null) {
				continue;
			}
