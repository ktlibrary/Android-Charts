...
public class LineChart extends GridChart {
	...
	protected void drawLines(Canvas canvas) {
		...
		for (int i = 0; i < linesData.size(); i++) {
			...
			if (axisYPosition == AXIS_Y_POSITION_LEFT) {
				...
				for (int j = lineDataSize - 1; j >= 0; j--) {
					float value = lineData.get(j).getValue();
					// calculate Y
					float valueY = (float) ((1f - (value - minValue)
							/ (maxValue - minValue)) * getDataQuadrantPaddingHeight())
	...
	protected void initAxisY() {
		...
		for (int i = 0; i < this.getLatitudeNum(); i++) {
			value = String.valueOf((int) Math.floor(minValue + i
					* average));
...
//			}
		...
		value = String.valueOf((int) Math.floor(((int) maxValue)));
//		if (value.length() < super.getLatitudeMaxTitleLength()) {
//			while (value.length() < super.getLatitudeMaxTitleLength()) {
//				value = " " + value;
//			}
//		}
	...
	protected void initAxisX() {
		...
		if (null != linesData && linesData.size() > 0) {
			float average = maxPointNum / (float)this.getLongitudeNum();
			int index;
			...
			titleX.add(String.valueOf(linesData.get(0).getLineData().get(maxPointNum - 1).getDate()));
		}
